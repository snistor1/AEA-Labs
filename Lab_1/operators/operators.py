...
def upgrade(population: list) -> list:
    ...
    # crossover_indices = [i for i in range(len(population)) if np.random.rand() < NET_CROSSOVER_P]
    ...
    #     cut_point = np.random.randint(1, min(len(population[idx_1]), len(population[idx_2])))
    #     new_ind_1 = population[idx_1][:cut_point] + population[idx_2][cut_point:]
    #     new_ind_2 = population[idx_2][:cut_point] + population[idx_1][cut_point:]
    #     population[idx_1], population[idx_2] = copy.deepcopy(new_ind_1), copy.deepcopy(new_ind_2)
    # return population


def crossover(population: list):
    crossover_indices = np.arange(len(population))[
        np.random.rand(len(population)) < NET_CROSSOVER_P
    ]
    np.random.shuffle(crossover_indices)
    if len(crossover_indices) % 2 != 0:
        crossover_indices = np.delete(crossover_indices, 0)
    for idx_1, idx_2 in crossover_indices.reshape(-1, 2):
        cut_point = np.random.randint(1, min(len(population[idx_1]), len(population[idx_2])))
        new_ind_1 = population[idx_1][:cut_point] + population[idx_2][cut_point:]
        new_ind_2 = population[idx_2][:cut_point] + population[idx_1][cut_point:]
        population[idx_1], population[idx_2] = copy.deepcopy(new_ind_1), copy.deepcopy(new_ind_2)
